...
> 基本的差别为：MyISAM 类型不支持事务处理等高级处理，而 InnoDB 类型支持。MyISAM 类型的表强调的是性能，其执行数度比 InnoDB 类型更快，但是不提供事务支持，而 InnoDB 提供事务支持以及外键和行级锁等高级数据库功能。 
>
> MyISAM 优势：
>
> - 读性能强。
> - MyISAM 索引和数据分开，能加载更多索引，而 InnoDB 索引与数据绑定，体积大。
> - InnoDB 不保存行数。select count(*) 要遍历表。
